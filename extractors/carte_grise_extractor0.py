...
class CarteGriseExtractor:
    ...
    def has_extract_ar(self, line):
        """
        Vérifie si l'un des blocks de la ligne correspond à un champ
        ayant l'option extract_ar activée dans la configuration.
        """

        fields_conf = self.config["FIELDS_KEY"]

        for block in line:
            field_name = block.get("field_name")

            if field_name in fields_conf.keys():
                field_conf = fields_conf[field_name]

                return field_conf.get("extract_ar", False)
    ...
    def get_safe_block_xy(self, line_idx, block_idx, lines, img_w, img_h):
        ...
        y_safe = min(block_y, y_prev_max) - 5
        y_bottom = max(block_y_max, y_next_min) + 7
        y_safe = max(0, y_safe)
        ...
        x_safe = max(block_x - int(abs(block_x - x_prev_max)/5) , x_prev_max)
        x_right = min(block_x_max + int(abs(x_next_min - block_x_max)/5), x_next_min)
        ...
        for line_idx, line in enumerate(lines):
            if y_min < line[0]["y"] < y_max:
                for block_idx, block in enumerate(line):
                    ...
                    processed_cropped = ProcessImage(image=cropped).process("mode_cg_easyocr")
                    # Write zone to test
                    filename = f"zone_test_{line_idx}_{block_idx}.jpg"
                    cv2.imwrite("debug/" + filename, processed_cropped)
                    if self.has_extract_ar(line) or is_arabic(block['text']):
                        results = reader_ar.readtext(processed_cropped)
                    else:
                        results = reader_en.readtext(processed_cropped)
                    if not results:
                        continue
                    ...
                    block.update({
                        "text": "--" if text.strip() == "3" else text.strip(),
                        "x": new_x,
                        "y": new_y,
                        "width": new_w,
                        "height": new_h,
    ...
    def extract_value(self, merge_lines, idx, pos_fr, pos_ar, key_data: str, multiline, lang, pattern, image_width):
        ...
        new_value = Normalize.normalize_value(value, pattern)
        new_confidence = confidence * 1.20 if new_value != value else confidence
        # Normalise date
        if key_data.endswith("_date"):
            new_value = Normalize.normalize_date(new_value)
            new_confidence = 0.9 if new_value is not None else new_confidence * 0.8
        # Normalise matricul
        if key_data.endswith("_matriculate"):
            new_value = Normalize.normalize_matricule(new_value)
            new_confidence = 0.9 if new_value is not None else new_confidence * 0.8
        block_result = {
            "text": new_value if new_value is not None else value,
            ...
            for idk,key_data in enumerate(keys):
                ...
                if pos_fr != -1 or pos_ar != -1:
                    block_value_fr = self.extract_value(
                        merge_lines, idx, pos_fr, pos_ar, key_data,
    ...
    def extract(self, image_path, document_type):
        ...
        preprocessed_image, image_original = ProcessImage(image_path).process("mode_cg_pytesseract")
